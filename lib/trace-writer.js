...
TraceWriter.prototype.publish_ = function(projectId, json) {
  ...
  this.request_(requestParameters, function(err, response, body) {
    that.logger_.silly('request params', JSON.stringify(requestParameters).replace(/\n/g, ' '));
    that.logger_.silly('response', (JSON.stringify(response) || 'none').replace(/\n/g, ' '));
    that.logger_.silly('err', (JSON.stringify(err) || 'none').replace(/\n/g, ' '));
    if (err) {
