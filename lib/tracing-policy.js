...
module.exports = {
  createTracePolicy: function(config) {
    ...
    if (!config.filters) return basePolicy;

    // search for a filter to apply
    Object.keys(config.filters).forEach(function (filter) {
      if (config.filters[filter].length > 0)
        basePolicy = new FilterPolicy(basePolicy, config.filters);
    })
